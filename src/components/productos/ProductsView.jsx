...
const ProductsView = () => {
  ...
  const productColumns = [
    { header: "Nombre", field: "name", width: 70 },
    { header: "Precio", field: "price", width: 40, format: val => `$${val.toFixed(2)}` },
 ]
        ...
        <ul className="space-y-4 mb-6">
          {products.map(p => (
            <li key={p.id} className="flex justify-between items-center bg-white p-4 rounded-lg shadow hover:shadow-lg transition">
              <div>
                <p className="font-semibold text-lg text-purple-700">{p.name}</p>
