...
const UsersView = () => {
  ...
  if (loading) return <p className="text-center mt-10 text-gray-600">Cargando usuarios...</p>
  ...
  const userColumns = [
    { header: "Nombre", field: "name", width: 50 },
    { header: "Email", field: "email", width: 80 },
    { header: "Edad", field: "age", width: 20, format: val => String(val) }
  ]
        ...
        <ul className="space-y-4">
          {users.map(u => (
            <li key={u.id} className="flex justify-between items-center bg-white p-4 rounded-lg shadow hover:shadow-lg transition">
              <div>
                <p className="font-semibold text-lg text-purple-700">{u.name}</p>
                <p className="text-gray-600">{u.email}</p>
                <p className="text-gray-500">Edad: {u.age}</p>
              </div>
